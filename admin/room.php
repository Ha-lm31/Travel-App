<?php
session_start();
include '../config.php';
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> Admin</title>
    <!-- fontowesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" integrity="sha512-xh6O/CkQoPOWDdYTDqeRdPCVd1SpvCA9XXcUnZS2FmJNp1coAFzvtCN9BmamE+4aHK8yyUHUSCcJHgXloTyT2A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <!-- boot -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="css/room.css">
</head>

<body>
    <div class="addroomsection">
        <form action="" method="POST">
            <label for="troom">Type of Room :</label>
            <select name="troom" class="form-control">
                <option value selected></option>
                <option value="Superior Room">SUPERIOR ROOM</option>
                <option value="Deluxe Room">DELUXE ROOM</option>
                <option value="Guest House">GUEST HOUSE</option>
                <option value="Single Room">SINGLE ROOM</option>
            </select>

            <label for="bed">Type of Bed :</label>
            <select name="bed" class="form-control">
                <option value selected></option>
                <option value="Single">Single</option>
                <option value="Double">Double</option>
                <option value="Triple">Triple</option>
                <option value="Quad">Quad</option>
                <option value="Triple">None</option>
            </select>
            
            <label for="price">Price (per night):</label>
            <input type="number" name="price" class="form-control" required>
            
            <label for="city">City:</label>
            <input type="text" name="city" class="form-control" required>

            <button type="submit" class="btn btn-success" name="addroom">Add Room</button>
        </form>
        
        <?php
        if (isset($_POST['addroom'])) {
            $typeofroom = $_POST['troom'];
            $typeofbed = $_POST['bed'];
            $price = $_POST['price'];
            $city = $_POST['city'];

            $sql = "INSERT INTO room(type, bedding, price, city) VALUES ('$typeofroom', '$typeofbed', '$price', '$city')";
            $result = mysqli_query($conn, $sql);

            if ($result) {
                header("Location: room.php");
            } else {
                echo "<div class='alert alert-danger'>Error adding room: " . mysqli_error($conn) . "</div>";
            }
        }
        ?>
    </div>

    <div class="room">
        <?php
        $sql = "SELECT * FROM room";
        $re = mysqli_query($conn, $sql);
        
        if (!$re) {
            echo "<div class='alert alert-danger'>Error fetching rooms: " . mysqli_error($conn) . "</div>";
        } else {
            while ($row = mysqli_fetch_array($re)) {
                $id = $row['type'];
                $roomClass = '';
                
                // Determine CSS class based on room type
                if ($id == "Superior Room") {
                    $roomClass = 'roomboxsuperior';
                } else if ($id == "Deluxe Room") {
                    $roomClass = 'roomboxdelux';
                } else if ($id == "Guest House") {
                    $roomClass = 'roomboguest';
                } else if ($id == "Single Room") {
                    $roomClass = 'roomboxsingle';
                }
                
                echo "<div class='roombox $roomClass'>
                        <div class='text-center no-boder'>
                            <i class='fa-solid fa-bed fa-4x mb-2'></i>
                            <h3>" . htmlspecialchars($row['type']) . "</h3>
                            <div class='mb-1'><strong>Bed:</strong> " . htmlspecialchars($row['bedding']) . "</div>
                            <div class='mb-1'><strong>Price:</strong> $" . htmlspecialchars($row['price']) . "/night</div>
                            <div class='mb-1'><strong>City:</strong> " . htmlspecialchars($row['city']) . "</div>
                            <a href='roomdelete.php?id=". $row['id'] ."'><button class='btn btn-danger'>Delete</button></a>
                        </div>
                    </div>";
            }
        }
        ?>
    </div>

</body>

</html>